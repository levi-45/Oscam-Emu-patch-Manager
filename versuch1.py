...
TEXTS = {
    "en": {
        ...
        "update_available_msg": "A new version ({version}) is available. Update now?",
        # Option Buttons
    ...
    "de": {
        ...
        "update_available_msg": "Eine neue Version ({version}) ist verfügbar. Jetzt aktualisieren?",
        ...
        #Patch ordner leeren
        "temp_repo_deleted": "Temp-Repository gelöscht: {path}",
...
def create_patch(gui_instance=None, info_widget=None, progress_callback=None):
    ...
    try:
        ...
        try:
            ...
            diff = ""
            log("patch_create_failed", "error", error=f"Diff failed: {e.output}")

        if not diff.strip():
            diff = TEXTS.get(lang, {}).get("patch_create_no_changes", "# No changes")
        ...
        if header.strip():
            ...
            if lines:
                log("patch_version_from_header", "success", patch_version=lines[0])
...
class PatchManagerGUI(QWidget):
    ...
    def check_for_update_on_start(self):
        ...
        # Sicherstellen, dass das Widget da ist
        widget = getattr(self, "info_text", None)
        lang = getattr(self, "LANG", "DE")
        ...
        try:
            ...
            if lv > cv:
                ...
                # Erfolg melden über zentrale Methode
                if widget:
                    msg = TEXTS[lang].get("version_current", "Version {version} ist aktuell.").format(version=APP_VERSION)
                    self.append_info(widget, msg, "success")

        except Exception as e:
            if widget:
                msg = TEXTS[lang].get("update_check_failed", "Fehler bei Updateprüfung: {error}").format(error=str(e))
    ...
    def setup_option_buttons(self, parent_layout):
        ...
        for key, text_key, color, callback, *rest in button_defs:
            ...
            def make_callback(f):
                ...
                return lambda *_: (
                    f(gui_instance=self, info_widget=self.info_text, progress_callback=None) 
