...
def save_config(cfg_updates, gui_instance=None, silent=False):
    ...
    try:
        ...
        if gui_instance:
            # GUI-intern S3 / NCam Pfade setzen
            ...
            led_enabled = current_cfg.get("led_enabled", False)

            timer = getattr(gui_instance, "blink_timer", None)
            if timer:
                if not led_enabled or blink_speed >= 950:
                    timer.stop()
        ...
        if gui_instance:
            ...
            if not is_loading and not silent:
                ...
                msg = (
                    "✅ Einstellungen gespeichert" if not is_closing else "✅ Beendet & Gespeichert"
                )
...
def load_config(gui_instance=None):
    ...
    try:
        ...
        if len(str(cfg.get("EMUREPO", "")).strip()) < 8:
        ...
        if gui_instance:
            ...
            led_enabled = cfg.get("led_enabled", False)
            ...
            if timer:
                timer.setInterval(max(10, blink_speed))
                if led_enabled and blink_speed < 950:
                    if not timer.isActive():
                        timer.start()
                else:
                    timer.stop()
...
class GithubConfigDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        w = screen.width()
        h = screen.height()
        self.resize(int(w * 0.6), int(h * 0.6))  # 60% vom Bildschirm
...
class CinematicMatrixSplash(QWidget):
    ...
    def __init__(
        self, duration=5000, logo_path=None, lang="de", start_sound=None, end_sound=None
    ):
        super().__init__()
        ...
        texts = {
            "de": {
                "loading": "🚀 OSCam Patch Manager wird gestartet...",
                "ready": "✅ Bereit!",
            },
            "en": {
                "loading": "🚀 OSCam Patch Manager loading...",
                "ready": "✅ Ready!",
            },
        }
        ...
        self.setStyleSheet(
            "background-color: black; border-radius: 12px; border: 2px solid #222;"
        )
        screen = self.screen().availableGeometry()
        w = screen.width()
        h = screen.height()
    ...
    def animate_pingpong(self):
        self.lbl_pingpong.setText(
            f"{self.ping_pong[self.pp_index % len(self.ping_pong)]} {self.text_loading}"
        )
        self.pp_index += 1
...
class PatchManagerGUI(QWidget):
    DIFF_COLORS = globals().get("DIFF_COLORS", {})
    def __init__(self):
        ...
        # Haupt-Widget Hintergrund sofort setzen
        if saved_color in self.DIFF_COLORS:
            theme_colors = self.DIFF_COLORS[saved_color]
            bg = theme_colors.get("bg", "#2F2F2F")
            fg = theme_colors.get("fg", "#FFFFFF")
            self.setStyleSheet(f"QWidget {{ background-color: {bg}; color: {fg}; }}")

        # Farbschema auf Widgets anwenden
        if hasattr(self, "apply_color_scheme"):
            self.apply_color_scheme(saved_color)

        # 1️⃣ Timer erzeugen und Timeout verbinden
        self.blink_timer = QTimer(self)
        self.blink_timer.timeout.connect(self.animate_status_bar)

        # 2️⃣ Gespeicherten LED-Status und Speed holen
        self.led_enabled = self.current_config.get("led_enabled", True)
        self.blink_speed = self.current_config.get("blink_speed", 500)

        # 3️⃣ Checkbox synchronisieren
        if hasattr(self, "led_checkbox") and self.led_checkbox:
            self.led_checkbox.blockSignals(True)
            self.led_checkbox.setChecked(self.led_enabled)
            self.led_checkbox.blockSignals(False)
            self.led_checkbox.stateChanged.connect(
                lambda state: self.toggle_leds(state == Qt.CheckState.Checked)
            )

        # 4️⃣ Slider verbinden
        if hasattr(self, "slider_speed") and self.slider_speed:
            self.slider_speed.setValue(self.blink_speed)
            self.slider_speed.valueChanged.connect(self.update_blink_speed)

        # 5️⃣ LEDs korrekt initialisieren ohne Config-Save
        self._block_config_save = True
        self.toggle_leds(self.led_enabled)
        ...
        self.change_colors()
    def apply_color_scheme(self, color_name, exclude_widgets=None):
        """
        Wendet das Farbschema auf alle Widgets an.
        exclude_widgets: Liste von Widgets, die ihre Originalfarben behalten sollen.
        """
        from PyQt6.QtWidgets import QLabel, QTextEdit, QFrame, QCheckBox, QPushButton
        from PyQt6.QtCore import Qt, QTimer

        exclude_widgets = exclude_widgets or []

        theme_colors = self.DIFF_COLORS.get(
            color_name,
            {"bg": "#2F2F2F", "fg": "#FFFFFF"}
        )
        ...
        def recursive_apply(widget):
            ...
            if widget in exclude_widgets or widget is None:
                return
            ...
            if isinstance(widget, QLabel):
                ...
                widget.setStyleSheet(
                    ...
                    f"padding:5px 10px; font-weight:700; }}"
                    f"QCheckBox::indicator {{ width:16px; height:16px; "
                    f"border:1px solid {fg}; border-radius:3px; }}"
                    f"QCheckBox::indicator:checked {{ background-color:{fg}; }}"
                )
                ...
                widget.setStyleSheet(
                   ...
                   f"padding:6px; font-weight:700; font-size:13pt;"
                )
        ...
        # optionaler Systemcheck
        if hasattr(self, "run_full_system_check"):
            QTimer.singleShot(
                2000,
                lambda: self.run_full_system_check(clear_log=False)
            )
        from PyQt6.QtCore import QTimer
        QTimer.singleShot(0, self._final_resize_fix)
    def _final_resize_fix(self):
        self.adjustSize()
        self.showFullScreen()
    # =====================================================================
    ...
    def toggle_leds(self, enable: bool, speed: int = None):
        """
        ...
        # Falls kein Speed übergeben, vom Slider holen oder Default
        if speed is None:
        ...
        if hasattr(self, "blink_timer") and self.blink_timer:
            if enable:
                if 10 <= speed < 950:
                    # Timer starten oder Interval aktualisieren
        ...
        # Config speichern, falls nicht blockiert
        if not getattr(self, "_block_config_save", False):
            if "save_config" in globals():
                save_config({"led_enabled": enable, "blink_speed": speed},
                            gui_instance=self, silent=True)
    ...
    def update_blink_speed(self, value):
        """
        User-LEDs blinken lassen, System-LEDs bleiben statisch.
        0 = OFF, >=950 = STATIC, sonst blinkend (nur wenn LEDs global aktiviert sind)
        """
        ...
        # --- LED-Logik ---
        is_led_globally_enabled = getattr(self, "led_enabled", True)
        ...
        self.current_config["led_enabled"] = led_on
        if "save_config" in globals():
            save_config({"blink_speed": value, "led_enabled": led_on}, gui_instance=self, silent=True)

        # --- Checkbox synchronisieren ---
        if hasattr(self, "led_checkbox"):
            self.led_checkbox.blockSignals(True)
            self.led_checkbox.setChecked(led_on)
            self.led_checkbox.blockSignals(False)
